...
for iExperimentType = 1:length(experimentFilenames)
	...
	score = NaN(length(experimentStruct),3);
	scoreTime = NaN(length(experimentStruct),6,2);
	for i=1:length(experimentStruct)
		posCamJoints2D = [experimentStruct(i).dataCam.camPos.(joints{1}).pos2D experimentStruct(i).dataCam.camPos.(joints{2}).pos2D];
		posCamJoints3D = [experimentStruct(i).dataCam.camPos.(joints{1}).pos3D experimentStruct(i).dataCam.camPos.(joints{2}).pos3D];
		score(i,1) = find_shift_and_alignment(posCamJoints2D, experimentStruct(i).dataIMU.(IMUlocation).quat, experimentStruct(i).dataCam.params.cam.intrinsicMat, 0);
		score(i,2) = computeSimilarityScoreBaselineAccel(experimentStruct(i).dataIMU.(IMUlocation), posCamJoints3D, baselineAccelAlphaJoint);
		score(i,3) = computeSimilarityScoreBaselineOrientation(experimentStruct(i).dataIMU.(IMUlocation).quat, posCamJoints3D);		
	end
	h = figure('Name', xLabelStr);
	stem(cell2mat({experimentStruct.(experimentVariable)}), score);
	legend(legendStr);
...
% 		for iSegment = 1:N
	...
	h = figure('Name', xLabelStr);
	plotBoxPlot(score, windowLengths, legendStr);
	xlabel(xLabelStr); ylabel('Score');
