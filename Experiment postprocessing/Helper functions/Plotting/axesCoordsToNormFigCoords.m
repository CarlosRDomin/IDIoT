function varargout = axesCoordsToNormFigCoords(varargin)
% axesCoordsToNormFigCoords  Convert data space units into normalized figure units. 
%
% [Xf, Yf] = axesCoordsToNormFigCoords(X, Y) converts X,Y coordinates from
% data space to normalized figure units, using the current axes.  This is
% useful as input for ANNOTATION.  
%
% POSf = axesCoordsToNormFigCoords(POS) converts 4-element position vector, POS from
...
%
% [Xf, Yf] = axesCoordsToNormFigCoords(HAX, X, Y) converts X,Y coordinates from
% data space to normalized figure units, on specified axes HAX.  
%
% POSf = axesCoordsToNormFigCoords(HAX, POS) converts 4-element position vector, POS from
...
% 		xa = [1.6 2]*pi;
% 		ya = [0 0];
% 		[xaf,yaf] = axesCoordsToNormFigCoords(xa,ya);
% 		annotation('arrow',xaf,yaf)
%
%       % Add an annotation requiring a position vector
% 		pose = [4*pi/2 .9 pi .2];
% 		posef = axesCoordsToNormFigCoords(pose);
...
% 		hAx2 = subplot(212);
% 		plot(t,-s);ylim([-1.2 1.2])
% 		[xaf,yaf] = axesCoordsToNormFigCoords(hAx1,xa,ya);
% 		annotation('arrow',xaf,yaf)
% 		pose = [4*pi/2 -1.1 pi .2];
% 		posef = axesCoordsToNormFigCoords(hAx2,pose);
% 		annotation('ellipse',posef)
...
% Copyright 2006-2014 The MathWorks, Inc
	...
	if length(varargin)==1	% Must be 4 elt POS vector
		...
		[axWhich, axPos] = deal(varargin{:});
		if ~any(strcmpi(axWhich, {'x', 'y'}))
			error('Last 2 arguments need to be a string indicating the axis ("x" or "y") followed by the position in that axis');
		end
	end
	...
	%% Transform data
	if exist('axWhich', 'var')
		if strcmpi(axWhich, 'x'), inds=[1 3 1]; norm=axwidth; else, inds=[3 4 2]; norm=axheight; end
		varargout{1} = (axPos-axlim(inds(1)))*axpos(inds(2))/norm + axpos(inds(3));
	else
