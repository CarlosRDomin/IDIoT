...
def NMS(heatmaps, upsampFactor=1., bool_refine_center=True, bool_gaussian_filt=False):
    """
    ...
    for joint in range(NUM_JOINTS):
        ...
        peak_coords = find_peaks(map_orig)
        ...
        for i, peak in enumerate(peak_coords):
            if bool_refine_center:
                ...
                # more than one peak per joint!) -> For now, skip it (it's accurate enough)
                ...
                refined_center = [0, 0]
                peak_score = map_orig[tuple(peak[::-1])]  # Flip peak coordinates since they are [x,y] instead of [y,x]
...
def find_connected_joints(paf_upsamp, joint_list_per_joint_type, num_intermed_pts=10):
    """
    ...
    for limb_type in range(NUM_LIMBS):
        ...
        if len(joints_src)==0 or len(joints_dst)==0:
            ...
            limb_intermed_coords[2,:] = paf_xy_coords_per_limb[limb_type][0]  # Specify the paf index that contains the x-coord of the paf for this limb
            limb_intermed_coords[3,:] = paf_xy_coords_per_limb[limb_type][1]  # And the y-coord paf index
            for i,joint_src in enumerate(joints_src):
                for j,joint_dst in enumerate(joints_dst):  # Try every possible joints_src[i]-joints_dst[j] pair and see if it's a feasible limb
                    limb_dir = joint_dst[:2] - joint_src[:2]  # Subtract the position of both joints to obtain the direction of the potential limb
                    limb_dist = np.sqrt(np.sum(limb_dir**2)) + 1e-8  # Compute the distance/length of the potential limb (norm of limb_dir)
                    limb_dir = limb_dir/limb_dist  # Normalize limb_dir to be a unit vector

                    limb_intermed_coords[1,:] = np.round(np.linspace(joint_src[0], joint_dst[0], num=num_intermed_pts))  # Linearly distribute num_intermed_pts points from the x coordinate of joint_src to the x coordinate of joint_dst
                    ...
                    criterion1 = (np.count_nonzero(score_intermed_pts>param['thre2']) > 0.8*num_intermed_pts)  # Criterion 1: At least 80% of the intermediate points have a score higher than thre2
                    criterion2 = (score_penalizing_long_dist > 0)  # Criterion 2: Mean score, penalized for large limb distances (larger than half the image height), is positive
                    if criterion1 and criterion2:
                        connection_candidates.append([i, j, score_penalizing_long_dist, score_penalizing_long_dist + joint_src[2] + joint_dst[2]])  # Last value is the combined paf(+limb_dist) + heatmap scores of both joints

            connection_candidates = sorted(connection_candidates, key=lambda x: x[2], reverse=True)  # Sort connection candidates based on their score_penalizing_long_dist
            connections = np.empty((0,5))
            max_connections = min(len(joints_src), len(joints_dst))  # There can only be as many limbs as the smallest number of source or destination joints (eg: only 2 forearms if there's 5 wrists but 2 elbows)
            for potential_connection in connection_candidates:  # Traverse all potential joint connections (sorted by their score)
                i, j, s = potential_connection[0:3]
                if i not in connections[:,3] and j not in connections[:,4]:  # Make sure joints_src[i] or joints_dst[j] haven't already been connected to other joints_dst or joints_src
                    connections = np.vstack([connections, [joints_src[i][3], joints_dst[j][3], s, i, j]])  # [joint_src_id, joint_dst_id, limb_score_penalizing_long_dist, joint_src_index, joint_dst_index]
                    if len(connections) >= max_connections:  # Exit if we've already established max_connections connections (each joint can't be connected to more than one joint)
                        break
...
def group_limbs_of_same_person(connected_limbs, joint_list):
    ...
    for limb_type in range(NUM_LIMBS):
            ...
            for limb_info in connected_limbs[limb_type]:
                ...
                if len(person_assoc_idx) == 1:  # If one of the joints has been associated to a person, and either the other joint is also associated with the same person or not associated to anyone yet:
                    person_limbs = person_to_joint_assoc[person_assoc_idx[0]]
                    if person_limbs[joint_dst_type] != limb_info[1]:  # If the other joint is not associated to anyone yet,
                        person_limbs[joint_dst_type] = limb_info[1]   # Associate it with the current person
                        person_limbs[-1] += 1                                  # Increase the number of limbs associated to this person
                        person_limbs[-2] += joint_list[limb_info[1].astype(int), 2] + limb_info[2]  # And update the total score (+= heatmap score of joint_dst + score of connecting joint_src with joint_dst)
                    ...
                    if not membership.any():  # If both people have no same joints connected, merge them into a single person
                        person1_limbs[:-2] += (person2_limbs[:-2] + 1)  # Update which joints are connected
                        person1_limbs[-2:] += person2_limbs[-2:]  # Update the overall score and total count of joints connected by summing their counters
                        person1_limbs[-2] += limb_info[2]  # Add the score of the current joint connection to the overall score
                    ...
                    row = -1*np.ones(20)  # Initialize person info to all -1 (no joint associations)
                    row[joint_src_type] = limb_info[0]  # Store the joint info of the new connection
    ...
    for person_id,person_info in enumerate(person_to_joint_assoc):
        if person_info[-1]<3 or person_info[-2]/person_info[-1]<0.2:
            ...
            person_info[-1] = person_id  # Use last column as ID (instead of number of limbs found)
    ...
    # Instead, we treat the set of people as a list (fast to append items) and only convert to np.array at the end
    return np.asarray(person_to_joint_assoc) if len(person_to_joint_assoc)>0 else np.empty((0,1))


def plot_pose(img_orig, joint_list, person_to_joint_assoc, bool_fast_plot=True, plot_ear_to_shoulder=False):
    canvas = np.ascontiguousarray(img_orig)  # Make sure the ait's contiguous so we can draw onto it

    for joint in joint_list:  # Draw circles at every joint
        cv2.circle(canvas, tuple(joint[0:2].astype(int)), 4, colors[int(joint[-1])], thickness=-1)

    # to_plot is the location of all joints found overlaid on top of the
    # original image
    to_plot = None if bool_fast_plot else canvas.copy()  # cv2.addWeighted(img_orig, 0.3, canvas, 0.7, 0)
    ...
    for limb_type in range(which_limbs_to_plot):
        for person_joint_info in person_to_joint_assoc:
            ...
            if not bool_fast_plot: canvas = cv2.addWeighted(canvas, 0.4, cur_canvas, 0.6, 0)

    return to_plot, canvas


def paf_to_pose(img_orig, heatmaps, pafs, bool_plot=True):
    # Bottom-up approach:
    # Step 1: find all joints in the image (organized by joint type: [0]=nose, [1]=neck...)
    joint_list_per_joint_type = NMS(heatmaps, img_orig.shape[0] / float(heatmaps.shape[0]))
    ...
    # (Step 4): plot results
    to_plot, canvas = plot_pose(img_orig, joint_list, person_to_joint_assoc) if bool_plot else None, None
...
def people_to_pose(people_list):
    ...
    joint_list = []
    count = 0
    person_to_joint_assoc = np.empty((len(people_list), people_list.shape[1]+2))
